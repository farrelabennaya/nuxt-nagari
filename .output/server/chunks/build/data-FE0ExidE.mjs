...
import { reactive, ref, withCtx, createVNode, useSSRContext } from 'vue';
  ...
  methods: {
    async fetchUsers() {
      try {
        ...
        this.counter = 0;
        this.userList = data2.users.filter((user) => user.role.name !== "Admin").map((user) => ({
          ...user,
          number: ++this.counter
        }));
      } catch (error) {
    ...
    async editUser() {
      ...
      try {
        ...
        if (!response.ok) {
          if (data2.errors) {
            this.errors.value = data2.errors;
          } else {
            throw new Error(data2.message || "Failed to edit user");
          }
        } else {
...
function _sfc_ssrRender(_ctx, _push, _parent, _attrs, $props, $setup, $data, $options) {
  ...
  _push(`</div></main><div class="relative overflow-x-auto ml-5 mr-5"><table class="w-full text-sm text-left text-gray-500 rtl:text-right dark:text-gray-400"><thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400"><tr><th scope="col" class="px-6 py-3">No</th><th scope="col" class="px-6 py-3">Nama</th><th scope="col" class="px-6 py-3">Username</th><th scope="col" class="px-6 py-3">Role</th><th scope="col" class="px-6 py-3 flex justify-center">Aksi</th></tr></thead><tbody><!--[-->`);
  ssrRenderList($data.userList, (user) => {
    _push(`<tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700"><th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">${ssrInterpolate(user.number)}</th><td class="px-6 py-4">${ssrInterpolate(user.name)}</td><td class="px-6 py-4">${ssrInterpolate(user.username)}</td><td class="px-6 py-4">${ssrInterpolate(user.role.name)}</td><td class="flex justify-center items-center"><button class="flex p-1.5 border border-yellow-500 me-2 mt-2 rounded-lg hover:rounded-xl mb-2 hover:bg-yellow-100 transition-all duration-300 text-yellow-500"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></button><button class="flex p-1.5 border border-red-500 me-2 mt-2 rounded-lg hover:rounded-xl mb-2 hover:bg-red-100 transition-all duration-300 text-red-500"><svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M5 7h14m-9 3v8m4-8v8M10 3h4a1 1 0 0 1 1 1v3H9V4a1 1 0 0 1 1-1ZM6 7h12v13a1 1 0 0 1-1 1H7a1 1 0 0 1-1-1V7Z"></path></svg></button></td></tr>`);
  });
  _push(`<!--]--></tbody></table></div>`);
  if ($data.isEditModalOpen) {
    _push(`<div class="fixed inset-0 flex items-center justify-center z-50 bg-gray-500 bg-opacity-75"><div class="bg-white p-6 rounded-lg shadow-lg w-full max-w-sm"><h3 class="text-xl font-semibold mb-4">Edit User</h3><form><div class="mb-4"><label for="edit-name" class="block text-sm font-medium text-gray-700">Name</label><input id="edit-name"${ssrRenderAttr("value", $data.editForm.name)} type="text" class="block p-2.5 w-full text-sm text-gray-900 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 mt-2">`);
    if ($data.errors.name) {
      _push(`<span class="text-red-500">${ssrInterpolate($data.errors.name[0])}</span>`);
    } else {
      _push(`<!---->`);
    }
    _push(`</div><div class="mb-4"><label for="edit-username" class="block text-sm font-medium text-gray-700">Username</label><input id="edit-username"${ssrRenderAttr("value", $data.editForm.username)} type="username" class="block p-2.5 w-full text-sm text-gray-900 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 mt-2">`);
    if ($data.errors.username) {
      _push(`<span class="text-red-500">${ssrInterpolate($data.errors.username[0])}</span>`);
    } else {
      _push(`<!---->`);
    }
    _push(`</div><div class="mb-4"><label for="edit-role" class="block text-sm font-medium text-gray-700">Role</label><select id="edit-role" class="block p-2.5 w-full text-sm text-gray-900 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 mt-2"><!--[-->`);
    ...
    if ($data.errors.role_id) {
      _push(`<span class="text-red-500">${ssrInterpolate($data.errors.role_id[0])}</span>`);
    } else {
      _push(`<!---->`);
    }
    _push(`</div><div class="flex justify-start mb-4 space-x-2"><button type="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-1 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"${ssrIncludeBooleanAttr($data.isLoading) ? " disabled" : ""}>`);
    if ($data.isLoading) {
