...
function registerServerBundle(options, nuxt, serverBundle) {
    ...
    async getContents() {
      ...
      const collectionsValues = collections.map((collection) => {
        if (typeof collection === "string") {
          ...
          const path = isFullCollectionExists ? `@iconify/json/json/${collection}.json` : `@iconify-json/${collection}/icons.json`;
...
function registerClientBundle(options, nuxt) {
...
const module = defineNuxtModule({
  ...
  async setup(options, nuxt) {
    ...
    if (serverBundle === "auto") {
      serverBundle = nuxt.options.dev ? "local" : KEYWORDS_EDGE_TARGETS.some(
